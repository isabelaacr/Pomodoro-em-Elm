--Pomodoro 
--Técnica para gerenciamento de tempo para aumentar a concentração e produtividade
--Esse código tem como objetivo contar 25 minutos e possuir um intervalo de 5 minutos de descanso
    ...
    case msg of
        Tick ->
            if model.cont == 0 then --Verifica se o contador chegou a zero
                ( { model | clockState = Resting, cont = 300 } 
                , Cmd.none
                )
